...
cv2.createTrackbar('L - s', 'panel', 21, 255, nothing)
...
while True:
    ...
    bg = np.dsplit(bg, bg.shape[-1])[2]
    ret, thresh = cv2.threshold(bg, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    ...
    bg = cv2.GaussianBlur(bg, (11, 11), 0)
    bg = cv2.medianBlur(bg, 15)
    bg = cv2.cvtColor(cv2.cvtColor(bg, cv2.COLOR_HSV2BGR), cv2.COLOR_BGR2GRAY)
    ret, thresh = cv2.threshold(bg, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
